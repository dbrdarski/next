...
| `analyzer::safety` — `BodySafe(instance, I)` | **WORKING, not yet wired** | Fact + assume-and-check discharge + **§5's partition rule** (verify per region-table row, each narrowed to `I ∩ row.region` via `region::select`). `countDown` over its declared domain now **proves by induction** — no reaching fixpoint, no widening. **Not yet the wired path**: `analyze_apply` still calls `bodycheck::body_summary`, which is why nine pins are unmoved. Replacing it is T1.4 |
...
| Suite | Result |
|---|---|
| `cargo test --lib` | **379 passed, 0 failed, 9 ignored** (4 parked blockers + 5 remaining false positives) |
