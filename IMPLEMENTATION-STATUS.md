...
| `analyzer::safety` — `BodySafe(instance, I)` | **PARTIAL, sound as far as it goes** | The fact + assume-and-check discharge are built and native (`I` from the call site; recursion resolves through the assumption, never unfolds). **A non-discharged call still falls back to the quarantined `bodycheck`**, so results that depend on that path inherit its status. The four blockers are **not** closed |
...
| **1b** coarse recursive target (**false rejection**) | `bodycheck.rs:213` binds the parameter to the **row region** (`> 0`) instead of the reaching domain (`Equals(1)`), so `x-1` becomes "anything above −1" and manufactures inputs like `−0.5` that reach `x + "s"` | the **finite row-set bound** for a recursive call the fact does not cover (C§13.2 / GR-03). `safety.rs` already discharges the *covered* case |
...
| Suite | Result |
|---|---|
| `cargo test --lib` | **378 passed, 0 failed, 10 ignored** (4 parked blockers + 6 false positives exposed by the severity ruling) |
