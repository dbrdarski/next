...
| Suite | Result |
|---|---|
| Unit tests (`cargo test --lib`) | **361 passed, 0 failed, 0 ignored** |
...
| **Grounding v1** (termination bound) | **v0.5 patch 0.5.1** | 🟡 | **design-closed** (compendium 1.0.18); GR-01…30, Phase GR. **G-1..G-7 built** (`analyzer/grounding.rs`, three-voiced): numeric descent (GR-05), **constant-drift refutation** (GR-23a — any drift: descending/ascending/period-1 self-loop), program-expressed linear measures (§6 — `2a+b`), lexicographic (§5 — path-sensitive), and structural descent (§2b — tuple peel). Owed: point-base/Ackermann (GR-18 grid+domain), peel-k grid, general closed-orbit cycle (GR-11), §4 exact-chains, §8 WorldDecided, mutual SCC; then wiring (the swap gate); §16 discharge. *ACCEPTED pending author stamp* |
