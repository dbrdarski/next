...
| Suite | Result |
|---|---|
| Unit tests (`cargo test --lib`) | **356 passed, 0 failed, 0 ignored** |
...
| **Grounding v1** (termination bound) | **v0.5 patch 0.5.1** | 🟡 | **design-closed** (compendium 1.0.18); GR-01…30, Phase GR. **G-1..G-5 built** (`analyzer/grounding.rs`, three-voiced): descent (GR-05), drift-away refutation (GR-23a, parity-split), program-expressed linear-measure descent (§6 GR-15a/16 — `2a+b`), and **lexicographic descent** (§5 GR-13/14 — path-sensitive; the reset pattern grounds, floors from path guards; relational floors Unproven). Owed: point-base/Ackermann (GR-18 grid+domain), §2b structural (tuple length), §7 closed-orbit, §4 exact-chains, §8 WorldDecided, mutual SCC; then wiring (the swap gate); §16 discharge. *ACCEPTED pending author stamp* |
