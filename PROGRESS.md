...
**Snapshot:** 2026-07-26 · Archive7 correction — body safety over actual call edges (param callees, edge domains) + dead-arm elimination.
...
| Suite | Result |
|---|---|
| Unit tests (`cargo test --lib`) | **314 passed, 0 failed, 0 ignored** |
...
| Body safety over **actual call edges** (Archive7): follows abstract applications (param/local callees resolved from the value; each callee over its edge domain), `SAFETY_STACK` cutoff; + `analyze_match` **dead-arm elimination** (proven-empty region / proven-false guard skipped; proven-true guard consumes) | Archive7 | ✅ §11 gate (param callee reject; edge-domain reject; narrowed dead branch accept); 🟡 `InstanceBodySummary` unification + neutral `semantics::*` owed |
...
| 2026-07-26 | (this) | Archive7 correction: `body_safety` over actual call edges (`SAFETY_STACK` cutoff — param callees, edge domains) + `analyze_match` dead-arm elimination; §11 adversarial gate | “Archive7 correction: body safety over actual call edges” |
