...
| Suite | Result |
|---|---|
| Unit tests (`cargo test --lib`) | **354 passed, 0 failed, 0 ignored** |
...
| **Grounding v1** (termination bound) | **v0.5 patch 0.5.1** | 🟡 | **design-closed** (compendium 1.0.18); GR-01…30, Phase GR. **G-1..G-4 built** (`analyzer/grounding.rs`, three-voiced): descent (GR-05 — `countDown`/`factorial` on `GE(0)∧Mod(1,0)`), drift-away refutation (GR-23a — specimen 12, parity-split), and **program-expressed linear-measure descent** (§6 GR-15a/16 — half-line stop `E ⋈ c` with substitute-and-normalize; `2a+b` grounds where no single arg descends; relational two-varying stops [permanent]-unprovable). Owed: point-base compound (GR-18 range), §5 lex, §7 closed-orbit, §4 exact-chains, §8 WorldDecided; then wiring (the swap gate); §16 discharge. *ACCEPTED pending author stamp* |
